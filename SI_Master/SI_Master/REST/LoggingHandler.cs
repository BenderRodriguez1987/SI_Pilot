...
namespace SI_Master.REST {
    public class LoggingHandler : DelegatingHandler {
		...
		async protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken) {
			...
			string path = baseUri.PathAndQuery.Replace("%2F", "/");
			var resultUri = new Uri(baseUri.Scheme + "://" + baseUri.Host + ":" + baseUri.Port + path);
			request.RequestUri = resultUri;
			...
			Debug.WriteLine($"{msg} URL: {req.RequestUri.Scheme}://{req.RequestUri.Host}:{req.RequestUri.Port}{req.RequestUri.PathAndQuery}");
